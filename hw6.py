...
top_rated_content = top_rated_query.filter(MoviesTop.ratings_num > 15).filter(MoviesTop.ratings_avg > 3.5)
top_rated_content = top_rated_content.order_by(MoviesTop.ratings_avg.desc())
top_rated_content_ids = [
    i[0] for i in top_rated_content.values(MoviesTop.movieid)
]
# --------------------------------------------------------------
...
top_5_tags = tags_df.groupby('tags').count().reset_index().sort_values('movieid', ascending = False)
top_5_tags = top_5_tags.rename(index = str, columns = {'movieid': 'num_of_films'})
